...
{ 
    ...
    void OnPostRender()
    {
         if (PicoVRManager.SDK.VRModeEnabled)
        {
              PVRPluginEvent.IssueWithDatapc(PicoVRManager.SDK.currentDevice.eyeTextures[PicoVRManager.SDK.currentDevice.currEyeTextureIdx + (int)(PicoVRManager.Eye.LeftEye) * 3].GetNativeTexturePtr(), PicoVRManager.SDK.currentDevice.eyeTextures[PicoVRManager.SDK.currentDevice.currEyeTextureIdx + (int)(PicoVRManager.Eye.RightEye) * 3].GetNativeTexturePtr());
         }
    }
#endif
#if UNITY_EDITOR
